...
## ユーザー確認前の必須手順（配布更新）
- ユーザーへ「確認してください」「レビューお願いします」を依頼する前に、**必ず全体ビルド更新とグローバルインストール更新を実施**する。
- `build` / `build:native:*` を実行した場合は、確認依頼の有無に関わらず **同セッション内で必ず `npm pack` と `npm install -g` によるグローバルコマンド更新まで行う**。
- 最低実施順は `app` で **build → native bundle build → npm pack → global install** とする。
- 実施後に、利用した確認コマンド（例: `easy-openclaw --version`）をユーザーへ明示してから確認依頼する。
